...
import SoftImageInput from "@/components/SoftImageInput.vue";
...
export default {
  ...
  computed: {
    ...
    preview() {
      ...
      else if (this.profile.profile_image) return this.profile.profile_image;
  ...
  methods: {
    ...
    async handleProfileChange() {
      ...
      try {
        if (this.file) {
          this.profileChange.profile_image = await this.$store.dispatch(
            "profile/uploadPic",
            {
              id: this.profileChange.id,
              pic: this.file,
            }
          );
        }
